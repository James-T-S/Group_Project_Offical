...
namespace Group_Project_Offical.Pages
{
	public class Doner_Donate_AccountModel : PageModel
	{
		...
		private async Task<int> InsertDonationAsync()
		{
			...
			// Store EstimatedValue in Donations table (this column exists based on your CSV)
			comm.CommandText = @"
		...
		private async Task InsertDonationItemAsync(int donationId, DonationItem item, string? photo)
		{
			...
			// FIXED: Removed EstimatedValue from DonationItems insert since column doesn't exist
			comm.CommandText = @"
                INSERT INTO DonationItems 
                (ItemName, Description, Size, Gender, Condition, Seasonality, PhotoURL, Status, DateProcessed, DonationID, CategoryID, AI_CategoryID, ProcessedBy) 
                VALUES 
                (@name, @desc, @size, @gender, @cond, @seasonality, @photo, @status, @dateProcessed, @donationId, @categoryId, @aiCategoryId, @processedBy);";
			...
			// REMOVED: EstimatedValue parameter since column doesn't exist in DonationItems
